<?php
/**
 * AJAX endpoint for updating a food item
 */
...
$errors = [];
if ($foodId <= 0) $errors[] = "Invalid food ID.";
...
try {
    ...
    $db->execute(
        "UPDATE `foods` SET `restaurant_id` = ?, `category_id` = ?, `name` = ?, `description` = ?, `price` = ?, `image_path` = ?, `is_popular` = ?, `is_latest` = ?, `is_available` = ? WHERE `id` = ?",
        [$restaurantId, $categoryId, $name, $description, $price, $imagePath, $isPopular, $isLatest, $isAvailable, $foodId]
    );
    ...
    error_log('Food update failed: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Unable to update food item.']);
}
